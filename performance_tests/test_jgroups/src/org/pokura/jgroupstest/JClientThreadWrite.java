...
public class JClientThreadWrite extends Thread {
	...
	public JClientThreadWrite(int index, int serverCount) {
		super();
		this.index = index;
	...
	private <T> void eventLoop() throws Exception {
		...
		while (true) {
			...
			// nf = disp.callRemoteMethodWithFuture(selectServerAddress(index),
			// readMethodCall,
			// RequestOptions.SYNC());
	...
	public static void main(String[] args) {
		...
		JClientThreadWrite[] ts;
		// first arg is starting no
		// second arg is no. of threads
		// third arg is server count
		if (args != null && args.length >= 2) {
			ts = new JClientThreadWrite[Integer.parseInt(args[1])];
		} else {
			ts = new JClientThreadWrite[1];
		}
		...
		for (int x=0; x < ts.length ; x++) {
			ts[x] = new JClientThreadWrite(initial + x, server);
			ts[x].start();
		}
