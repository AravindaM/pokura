...
public class JClientThreadRead extends Thread {
	...
	public JClientThreadRead(int index, int serverCount) {
		super();
		this.index = index;
	...
	private <T> void eventLoop() throws Exception {
		...
		tick.scheduleAtFixedRate(checkStats, 0, 5000);
		...
		while (true) {
			...
			nf = disp.callRemoteMethodWithFuture(selectServerAddress(index), readMethodCall, RequestOptions.SYNC());
			...
			// } else {
			// NotifyingFuture<RspList<Object>> nf;
			// nf = disp.callRemoteMethodsWithFuture(getServerList(),
			// writeMethodCall, RequestOptions.SYNC());
			// nf.setListener(new FutureListener<RspList<Object>>() {
			// @Override
			// public void futureDone(Future<RspList<Object>> future) {
			// try {
	...
	private Address selectServerAddress(int serverIndex) {
		...
		int selectedIndex = serverIndex % serverCount;
	...
	public static void main(String[] args) {
		...
		JClientThreadRead[] ts;
		// first arg is starting no
		// second arg is no. of threads
		// third arg is server count
		if (args != null && args.length >= 2) {
			ts = new JClientThreadRead[Integer.parseInt(args[1])];
		} else {
			ts = new JClientThreadRead[1];
		}
		...
		for (int x = 0; x < ts.length; x++) {
			ts[x] = new JClientThreadRead(initial + x, server);
			ts[x].start();
		}
